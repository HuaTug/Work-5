...
func ListComment(List comment.ListCommentRequest) ([]*comment.Comment, int64, error) {
	...
	err := mysql.Db.Model(&comment.Comment{}).Where("video_id=?", List.VideoId).Count(&total).Limit(int(List.PageSize)).Offset(int((List.PageNum - 1) * List.PageSize)).Find(&comments).Error
